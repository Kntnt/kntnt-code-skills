...
- The `coder` skill now loads the standard as a **lazy bootstrap** instead of profiling the project and reading every matching module up front. A skill trigger loads only `SKILL.md` and the standard's router (`lib/coding-standard/_index.md`); each topic module is pulled in only at the moment the working context proves its axis applies — `general.md` when code work begins, a language module when that language appears, a framework module when its markers surface — and the bootstrap stays resident so it keeps pulling more in as new languages or frameworks reveal themselves through the session. An empty project with no language cue loads nothing beyond the router. The standalone-script meta-rule (how to choose and package a script's language when detection finds nothing) moved out of `SKILL.md` into `skills/coder/standalone-scripts.md`, loaded on demand only when that case arises, so it no longer weighs on every trigger. No rules changed — only when each module is loaded.
