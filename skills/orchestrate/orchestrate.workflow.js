...
const integrate = async (record) => {
  ...
  const action = (merge
    ? `Land branch ${record.branch} on the default branch by fast-forwarding the DEFAULT branch to that branch's tip, WITHOUT ever checking out ${record.branch}. ${record.branch} is still checked out in the implementer's (or a fix round's) persisted worktree, and git refuses to check out one branch in two worktrees at once, so checking it out here would fail mechanically for a non-conflict reason. The default branch is checked out in exactly ONE worktree — this un-isolated main checkout — so run \`git merge --ff-only ${record.branch}\` from THAT worktree, on the default branch: it advances the default to the feature tip, creates NO merge commit, and updates the default's working tree so the landed code actually reaches disk. ` +
