...
- `--max-lenses=N` — **override** the level-derived per-issue verifier-panel size (1 lens at `XS`/`S`/`M`, 2 at `L`, 3 at `XL`; a risk-escalated issue can raise its own panel further), mirroring `--max-fix-rounds` exactly: a **per-run** override only, never a policy default. `N ≥ 1` is a plain, floor-respecting cap; `N = 0` is the **only** route to a zero panel — it skips just the per-issue independent verify stage while **test-first**, **red-before-green**, and the **mandatory integration review** all still hold (ADR-0003 §2; see *Rigor baseline* below for the risk precedence under `N = 0`).
