...
- **Every sub-agent reports in three buckets** to its caller: *Automatically tested* (what, at which layer), *Remaining for a human* (the irreducibly subjective checks automation cannot meaningfully make), and *Assumptions & blockers* — which also carries the **shared-symbol ripple report** (#36): when a change alters a shared symbol's contract or effective behaviour, especially beyond the files the task obviously owns, enumerate the affected call sites and state which were updated and which were not, so a silently incomplete refactor is a reported gap the orchestrator and verifier can act on, not one caught only at the mandatory integration review (a full extra review + remediation cycle).
...
It returns the dependency graph, the topologically ordered **waves** (issues in one wave are **dependency-independent** — free of any explicit `Blocked by` edge between them — and may run concurrently; that is dependency-independence only, not a promise the pair shares no other coupling, which is exactly what the implicit-relationship assessment below still checks before treating them as safe to run side by side), any dependency that points outside the scope, and the issues it excluded. A dependency cycle is a hard error it reports rather than guessing past. With `--plan`, present this and stop.
...
Beyond that computed graph, the orchestrator's own read of every in-scope issue and brief (step 1) additionally assesses two **implicit relationships** the issue text can reveal that `orchestrate.py plan` does not compute — the helper stays prose-blind by design, so this reasoning lives entirely in the LLM-side planning pass. **Same-wave file/module overlap**: when two issues placed in the same wave will rewrite the same seam, serialize the colliding pair — soft-order one after the other, or call for one-issue-per-slice — rather than let them run concurrently, because concurrent same-module rewrites conflict at integrate time (the #63 park this closes). The plan's `soft_notes` array — the non-directional "touches the same files as #N" phrases the helper already extracts — is first-class input to this assessment, not merely an audit-trail note in the output. **Cross-reference to a deliverable**: when an issue's text names another in-scope issue's not-yet-built command, symbol, or file with no explicit `Blocked by` edge between them, treat the reference as a missing dependency — order the referrer's wave after the provider's, or flag the missing hard edge. Both adjustments are silent-but-visible, exactly like the `no_brief` flag above: the orchestrator makes the call, then states at the confirm gate (step 3) what it reordered or serialized and why, so a human sees the reasoning rather than a fait accompli.
...
Show the plan from step 2 — scope, graph, waves (including any reordering or serialization from the file/module-overlap and deliverable-cross-reference assessment above, with the reason for each), and whether the run will merge or open pull requests — and wait for one confirmation. Per the decision-boundary map above (ADR-0001 §8), exactly three things surface at this single gate — **merge authority, the cost cap, and the go/no-go** — and everything else stays silent; the implicit-relationship reordering/serialization is visibility, like the `no_brief` flag, not a fourth confirmable decision. The **merge-vs-PR decision is shown here** but was *settled before* the gate by `--merge` or the project merge policy (see the operating contract); the gate displays it, it does not re-decide it. `--yes`, or a `--plan` you have already reviewed, skips the gate — and it waives **only the go/no-go**, never the merge decision: `--yes` alone still opens pull requests unless merge authority was granted separately. Beyond this gate the run does not stop, with one exception: integrating into the default branch happens only under `--merge` or a merge-granting project policy; otherwise the work lands as pull requests.
...
Same-wave file/module **overlap** and an unresolved deliverable **cross-reference** (the implicit-relationship assessment in step 2) are themselves reasons to serialize or reorder rather than run a wave fully concurrently — and for a genuinely **tightly-coupled** refactor, where every issue in the batch rewrites the same seam, prefer **one-issue-per-slice** over a wide concurrent wave, trading a slower run for a conflict-free one.
...
4. **Integrate, immediately.** Land each green issue the moment it is verified, before the next issue's work begins, in dependency order. Keep history linear: fast-forward the *default* branch to the feature branch's tip (`git merge --ff-only <feature>` from the default branch) — never merge the default branch into the feature branch, never create a merge commit there. Crucially, the feature branch is still checked out in its implementer's (or fix round's) persisted worktree, and git refuses one branch in two worktrees at once, so integrate advances the default *without ever checking out the feature branch*. Because each implementer creates its branch **fresh off the current default tip** (with `git checkout -B`, never the harness's stale run-start worktree base) and issues land serially, the feature branch is already a fast-forward ahead of the default, so no rebase replay is needed. In the rare case the default moved under a verified branch anyway, a genuine content conflict does **not** park the work outright: a **bounded reconcile** — rebase-and-resolve keeping both sides' concerns, then a targeted re-verify of only that resolution, then land — repairs it (merge mode only, capped by `maxFixRounds`), parking only when the re-verify blocks or the cap is hit. Landing per-issue makes partial progress durable: a stop between issues leaves every already-integrated issue on the default branch. Under the conservative default, open a pull request instead and leave the merge to the maintainer.
