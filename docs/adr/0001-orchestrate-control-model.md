...
>
> **Amended by [ADR-0005](0005-durable-landed-markers-and-close-at-integrate.md).** §7's inviolable floor — "close an issue only after independent verification *and* integration" — is unchanged in substance; that ADR pins *when* and *by whom* the close happens (at integrate time, by the integrate step) and carves one narrow exception to §7's "no sub-agent closes/pushes/merges" rule (the integrate step alone may post the durable landed-marker and, merge mode, close the issue). §5's mandatory integration review is unchanged; it now operates at run level over already-closed issues and never reopens one. This makes an interrupted run safe to restart across sessions. Again left as originally decided and cross-referenced, not rewritten.
>
> **Amended by [ADR-0006](0006-mid-run-milestone-heartbeat-and-the-reporter-role.md).** §7's "no sub-agent closes/pushes/merges" rule is widened a second time — beyond ADR-0005's mutating integrate exception — to admit a **second, strictly weaker** outward writer: a comment-only, mechanical-tier *reporter* that posts mid-run milestone comments (`started` / `implementation green` / `verification cleared` / `fix round k` / `parked`) and mutates no shared state. The §7 inviolable floor is untouched — the reporter never closes; integrate remains the sole *mutating* writer and the sole closer. Again left as originally decided and cross-referenced, not rewritten.
