...
**What `N = 0` removes, and what it keeps.** It removes *only* the independent per-issue second opinion. It keeps: **test-first** (the implementer still writes and runs its own tests); **red-before-green** (checked deterministically by `orchestrate.py redgreen`, not by a lens, so it holds even at 0); and the **mandatory integration review** (ADR-0001 §5's once-per-run adversarial pass, hotfix-capable in merge mode, which always runs). The name scopes itself: the integration review is not a "lens," so `--max-lenses=0` cannot touch it.
...
- **The inviolable per-issue-lens floor (§5) is now breachable** by `--max-lenses=0`. Mitigated by everything that survives the breach: the integration review and red-before-green always hold, the flag is per-run only, in-situ risk still escalates a lens, plan-time risk is shown at the gate, and `--max-lenses=0 --pr` is the easy human-gated escape.
